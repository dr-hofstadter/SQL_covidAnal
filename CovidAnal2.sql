...
--infectionRate date-wise
select location, date, population, max(total_cases) as highestinfectionRate, max((total_cases/population))*100 as PercentPopInfect
from portfolio1..coviddeath
...
--(rollingVac/population)*100 (we cannot use rollingVac without including it in CTE or Temp table because it is not present in the database)
from portfolio1..coviddeath dea
join portfolio1..covidvac vac
on dea.location=vac.location and dea.date=vac.date
)
select * , (rollingVac/population)*100 as percentPoPVaxxed
from PoV